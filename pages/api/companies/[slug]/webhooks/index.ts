...
const handlePOST = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  throwIfNotAllowed(companyMember, 'company_webhook', 'create');

  const { name, url, eventTypes } = validateWithSchema(
    webhookEndpointSchema,
    req.body
  );
  const app = await findOrCreateApp(companyMember.company.name, companyMember.company.id);
...
// Get all webhooks created by a company
const handleGET = async (req: NextApiRequest, res: NextApiResponse) => {
  const companyMember = await throwIfNoCompanyAccess(req, res);
  throwIfNotAllowed(companyMember, 'company_webhook', 'read');
...
const handleDELETE = async (req: NextApiRequest, res: NextApiResponse) => {
  const companyMember = await throwIfNoCompanyAccess(req, res);
  throwIfNotAllowed(companyMember, 'company_webhook', 'delete');
  ...
  if (app.uid != companyMember.company.id) {
    throw new ApiError(400, 'Bad request.');
  }
