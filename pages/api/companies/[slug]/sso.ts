...
// Get the SSO connection for the company.
const handleGET = async (req: NextApiRequest, res: NextApiResponse) => {
  const companyMember = await throwIfNoCompanyAccess(req, res);

  throwIfNotAllowed(companyMember, 'company_sso', 'read');
  ...
  const params =
    'clientID' in req.query
      ...
      : { tenant: companyMember.companyId, product: env.jackson.productId };
...
// Create a SSO connection for the company
const handlePOST = async (req: NextApiRequest, res: NextApiResponse) => {
  const companyMember = await throwIfNoCompanyAccess(req, res);

  throwIfNotAllowed(companyMember, 'company_sso', 'create');
  ...
  const connection = await sso.createConnection({
    ...
    tenant: companyMember.companyId,
  });
...
const handleDELETE = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  throwIfNotAllowed(companyMember, 'company_sso', 'delete');
