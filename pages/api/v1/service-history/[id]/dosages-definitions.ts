...
import { getDosageDefinitions } from 'models/dosageDefinition';
...
const handler = async (req: MobileAuthRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    console.error('Dosage Definitions API Error:', error);
...
const handleGET = async (req: MobileAuthRequest, res: NextApiResponse, companyId: string) => {
  const definitions = await getDosageDefinitions(companyId);
