...
const test = base.extend<MemberFixture>({
  ...
  memberPage: async ({ page }, use) => {
    const apiKeysPage = new MemberPage(page, company.slug);
    // eslint-disable-next-line react-hooks/rules-of-hooks
    await use(apiKeysPage);
  },
  secondUserJoinPage: async ({ page }, use) => {
    const joinPage = new JoinPage(page, secondUser, secondUser.company.name);
    await joinPage.goto();
    // eslint-disable-next-line react-hooks/rules-of-hooks
    await use(joinPage);
  },
...
test('Should be able to get the list of members', async ({
  ...
  await memberPage.companyMemberExists(user.name, user.email, 'OWNER');
});
...
test('New member should be able to accept the invitation', async ({
  ...
  await loginPage.gotoInviteLink(invitationLink, company.name);
  ...
  await loginPage.invitationAcceptPromptVisible(company.name);
...
test('Existing user should be able to accept the invitation', async ({
  ...
  await loginPage1.gotoInviteLink(invitationLink, company.name);
  ...
  await loginPage1.invitationAcceptPromptVisible(company.name);
...
test('Should be able to create invite using domain', async ({
  ...
  const loginPage1 = new LoginPage(page1);
  await loginPage1.gotoInviteLink(domainInviteLink, company.name);
...
test('Should not allow to invite a member with invalid domain', async ({
  ...
  await loginPage.gotoInviteLink(domainInviteLink, company.name);
